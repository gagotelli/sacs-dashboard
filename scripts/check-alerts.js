...
function evaluate() {
  ...
  (lic?.exhausted || []).forEach((model) => {
    alerts.push({
      key: `licence-pool:${model}`, severity: "warning",
      title: `No spare Meraki licences for ${model}`,
      detail: `Every licensed ${model} slot is in use — another one cannot be added until a licence is bought.`,
    });
  });
